...
const CARD = { background:"rgba(17,37,56,0.85)", border:"1px solid rgba(47,127,134,0.12)", boxShadow:"0 4px 24px rgba(0,0,0,0.4)" };

function FakeField({ label, placeholder, tall=false }: { label:string; placeholder:string; tall?:boolean }) {
  return (
    <div>
      <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color:"rgba(127,175,178,0.45)" }}>{label}</label>
      <div className={`w-full rounded-xl px-4 py-2.5 text-sm ${tall?"min-h-[80px]":""}`}
        style={{ background:"rgba(47,127,134,0.04)", border:"1px solid rgba(47,127,134,0.12)", color:"rgba(127,175,178,0.4)" }}>
        {placeholder}
      </div>
    </div>
  );
    ...
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <div className="relative overflow-hidden rounded-2xl p-6 sm:p-8 ximo-fade-up"
        ...
        <div className="relative flex items-start gap-4">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-2xl ximo-float"
            style={{ background:"rgba(47,127,134,0.15)", border:"1px solid rgba(47,127,134,0.25)" }}>🏷</div>
          <div>
            <h1 className="text-2xl font-black text-white sm:text-3xl" style={{ textShadow:"0 2px 20px rgba(47,127,134,0.4)" }}>Promociona con ximo</h1>
            <p className="mt-2 text-sm leading-relaxed max-w-lg" style={{ color:"rgba(127,175,178,0.6)" }}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {["Revisión garantizada","Sin spam","Solo marcas deportivas","Comunidad real"].map(tag => (
                <span key={tag} className="rounded-full px-3 py-1 text-[10px] font-bold"
                  style={{ border:"1px solid rgba(47,127,134,0.2)", background:"rgba(47,127,134,0.08)", color:"rgba(127,175,178,0.6)" }}>
                  ✓ {tag}
                </span>
              ))}
            </div>
      ...
      <div
        ...
        style={{ background: "rgba(47,127,134,0.06)", border: "1px dashed rgba(47,127,134,0.2)" }}
      >
        <div className="text-center">
          <p className="text-sm font-bold" style={{ color: "rgba(127,175,178,0.5)" }}>Panel de marca</p>
          <p className="mt-1 text-xs" style={{ color: "rgba(127,175,178,0.3)" }}>Dashboard interactivo próximamente</p>
        </div>
      </div>
          ...
          {steps.map(s => (
            <div key={s.n} className="rounded-2xl p-4 sm:p-5 ximo-lift" style={CARD}>
              <div className="flex gap-4">
                <span className="text-2xl font-black leading-none mt-0.5 shrink-0" style={{ color:"rgba(47,127,134,0.3)" }}>{s.n}</span>
                <div>
                  <p className="text-sm font-bold text-white">{s.title}</p>
                  <p className="mt-1 text-xs leading-relaxed" style={{ color:"rgba(127,175,178,0.5)" }}>{s.text}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {oppTypes.map(t => (
            <div key={t.label} className="rounded-2xl p-4 ximo-lift" style={CARD}>
              ...
              <p className="text-xs leading-relaxed" style={{ color:"rgba(127,175,178,0.5)" }}>{t.text}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="ximo-fade-up delay-200">
        ...
        <div className="rounded-2xl p-5 sm:p-6 space-y-4" style={CARD}>
          ...
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color:"rgba(127,175,178,0.45)" }}>Categoría</label>
            <PillSelect options={cats} />
          </div>
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color:"rgba(127,175,178,0.45)" }}>Formato del anuncio</label>
            <PillSelect options={fmts} />
          </div>
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color:"rgba(127,175,178,0.45)" }}>Tipo de oportunidad</label>
          ...
          <button type="button"
            className="w-full rounded-xl py-3 text-sm font-bold text-white transition-all duration-200 hover:scale-[1.02]"
            style={{ background:"linear-gradient(90deg,#2F7F86,#1F5F66)", boxShadow:"0 0 20px rgba(47,127,134,0.3)" }}>
            Enviar a revisión →
          </button>
        </div>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3 mb-4">
          {budgets.map(b => (
            <div key={b.price} className="rounded-2xl p-4 text-center ximo-lift"
              style={b.hot
                ? { border:"1px solid rgba(47,127,134,0.35)", background:"rgba(47,127,134,0.1)", boxShadow:"0 0 24px rgba(47,127,134,0.2)" }
                : CARD}>
              {b.hot && <span className="mb-2 inline-flex items-center rounded-full px-2 py-0.5 text-[9px] font-bold uppercase"
                style={{ background:"#2F7F86", color:"white" }}>Recomendado</span>}
              <p className="text-xl font-black text-white">{b.price}</p>
              <p className="mt-0.5 text-xs font-bold" style={{ color:"#7FAFB2" }}>{b.days}</p>
              <p className="mt-1.5 text-[10px] leading-relaxed" style={{ color:"rgba(127,175,178,0.5)" }}>{b.reach}</p>
              <p className="mt-0.5 text-[9px] italic" style={{ color:"rgba(127,175,178,0.3)" }}>alcance estimado</p>
            </div>
          ))}
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[{l:"Presupuesto",v:"$500 MXN"},{l:"Duración",v:"7 días"},{l:"Alcance estimado",v:"1,200–2,000"},{l:"Tipo de campaña",v:"Comunidad feed"}].map(i => (
              <div key={i.l} className="rounded-xl p-3 text-center"
                ...
                <p className="text-[10px] mt-0.5" style={{ color:"rgba(127,175,178,0.4)" }}>{i.l}</p>
              </div>
            ))}
          </div>
          <p className="mt-3 text-[10px] text-center" style={{ color:"rgba(127,175,178,0.3)" }}>
            Ubicación: Promociones filtradas dentro de Comunidad · Estimados de referencia
          </p>
          <button type="button"
            className="mt-3 w-full rounded-xl py-2.5 text-xs font-bold text-white transition-all duration-200 hover:scale-[1.01]"
            style={{ border:"1px solid rgba(47,127,134,0.2)", background:"rgba(47,127,134,0.1)", color:"#7FAFB2" }}>
      ...
      <div className="rounded-2xl p-4 sm:p-5 flex items-start gap-4 ximo-fade-up delay-400" style={CARD}>
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-lg"
          style={{ background:"rgba(47,127,134,0.12)", border:"1px solid rgba(47,127,134,0.2)" }}>🛡</div>
        <div>
          <p className="text-sm font-black text-white">Revisión por ximo</p>
          <p className="mt-1 text-xs leading-relaxed" style={{ color:"rgba(127,175,178,0.5)" }}>
