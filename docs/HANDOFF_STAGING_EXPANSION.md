...
- ✅ **Webhook RESUELTO Y VERIFICADO E2E** (era el último bloqueante): las entregas al Preview devolvían **401** por la **Vercel Deployment Protection** (login requerido; Stripe server-to-server no está logueado). Se resolvió con **Protection Bypass for Automation**: Manuel creó un secreto de bypass en Vercel (Settings → Deployment Protection) y yo agregué `?x-vercel-protection-bypass=<secreto>` a la URL del webhook en Stripe. Los previews siguen protegidos para navegadores; solo requests con el token (Stripe) pasan. **Verificado con curl** (sin token: 302 login; con token: `{"status":"ok"}`) y **E2E**: reenvié el `checkout.session.completed` fallido → el Preview respondió 200 → la suscripción quedó **`active`** en `ximo-staging` (`subscriptions.status='active'`, `sub_1TsYcK...`, `profiles.subscription_status='active'` por el trigger, evento en `processed_webhook_events`). El secreto de bypass NO se guarda en el repo (público); vive en Vercel (`VERCEL_AUTOMATION_BYPASS_SECRET`) y en la URL del webhook de Stripe. **Staging queda 100% funcional incluyendo activación de pagos.** Recordatorio: la URL del webhook es del branch preview; si se mergea/borra la rama, repuntarla.
