...
**Opcional para Manuel (solo si quiere flujos que usan secretos en staging):** en Vercel, agregar valores **solo-Preview** para `SUPABASE_SERVICE_ROLE_KEY` (secret key de `ximo-staging`, en Supabase → Settings → API Keys → Secret keys) y para el bundle de Stripe TEST (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_*`). No los puse yo porque no tecleo claves secretas en formularios. Sin ellos, registro/login/navegación en staging YA funcionan; solo el checkout de Stripe y las tareas server-side con service-role quedan deshabilitadas en Preview (de forma segura).
